...
namespace Tech.Areas.Admin.Controllers
{
    ...
    public class OrderController : Controller
    {
            ...
            foreach (var item in orderVM.orderDetail)
            {
                var sessionLineItem = new SessionLineItemOptions
                {
                    PriceData = new SessionLineItemPriceDataOptions
                    {
                        ...
                        UnitAmount = (long)(GetPrice(item) * 100)
                    },
                    Quantity = item.Count,
                };
        ...
        private double GetPrice(OrderDetail detail)
        {
            if (detail.Product.DiscountedPrice == 0 || detail.Product.DiscountedPrice == null)
            {
                return detail.Product.Price;
            }
            else
            {
                return detail.Product.DiscountedPrice;
            }
        }
        #region API CALLS
        [HttpGet]
